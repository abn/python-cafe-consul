...
class SessionedConsulAgent(LoggedObject, object):
    ...
    SESSION_RENEWAL_SECONDS = int(getenv('CONSUL_SESSION_RENEWAL_SECONDS', '75'))

    def __init__(self, name=None, renewal_interval=None, host=CONSUL_HOST, port=CONSUL_PORT,
                 token=CONSUL_TOKEN, scheme=CONSUL_SCHEME, dc=CONSUL_DC, verify=CONSUL_VERIFY, **kwargs):
        """
        :param name: session name to use
        :type name: str
        :param renewal_interval: interval (in seconds) in which a session should be renewed, this value is also used as
                                 the session ttl.
        :type renewal_interval: str
        """
        self.name = name if name is not None else self.SESSION_NAME
        self.ttl = renewal_interval if renewal_interval is not None else self.SESSION_RENEWAL_SECONDS
        self.consul = Consul(
    ...
    def start(self):
        """Start this instance by creating a new session and starting heartbeat renewals."""
    ...
    def stop(self):
        """Execute clean-up tasks, including stopping heartbeat (renewal) and session destruction."""
    ...
    def wait_for_ready(self, attempts=None, interval=3):
        """
        :param attempts: number of attempts before giving up, if None there is no giving up.
        :type attempts: int or None
        :param interval: interval (in seconds) between each check, if None, session ttl / 4 is used. (default=3)
    ...
    def session_create(self, retry=True):
        """
        Create a session, and set the internal `session_id` property. If an exception is encountered during creation,
        the operation will be reattempted again at half the ttl of the session itself if `retry` is `True`.
        ...
        try:
            ...
            self.logger.warning('session=%s creation failed, retrying reason=%s', self.session_id, e.message)
            if retry:
    ...
    def session_renew(self):
        ...
        try:
            ...
            self.logger.warning('session=%s renewal attempt failed reason=%s', self.session_id, e.message)
    ...
    def _lock(self, action, key, value=None):
        ...
        self.logger.debug('lock=%s action=%s session=%s value=%s', key, action, self.session_id, value)
        if not self.ready:
            self.logger.trace('lock=%s action=%s failed as consul agent is not ready', key, action)
            result = False
        else:
            result = yield self.consul.kv.put(key=key, value=value, **{action: self.session_id})
    ...
    def release_lock(self, key, value=None):
        ...
        if result:
            try:
                ...
                self.logger.warning('key=%s failed to delete reason=%s', key, e.message)
        defer.returnValue(result)
